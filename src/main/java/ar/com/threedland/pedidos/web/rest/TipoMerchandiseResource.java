...
public class TipoMerchandiseResource {
    ...
    public List<TipoMerchandise> getAllTipoMerchandises(@RequestParam(required = false) String filter) {
        if ("producto-is-null".equals(filter)) {
            log.debug("REST request to get all TipoMerchandises where producto is null");
            return StreamSupport
                .stream(tipoMerchandiseRepository.findAll().spliterator(), false)
                .filter(tipoMerchandise -> tipoMerchandise.getProducto() == null)
                .toList();
        }
