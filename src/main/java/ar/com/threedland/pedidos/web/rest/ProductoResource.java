...
/**
 * REST controller for managing {@link ar.com.threedland.pedidos.domain.Producto}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ProductoResource {
    ...
    public List<Producto> getAllProductos(@RequestParam(required = false) String filter) {
        if ("costoprducto-is-null".equals(filter)) {
            log.debug("REST request to get all Productos where costoPrducto is null");
            return StreamSupport
                .stream(productoRepository.findAll().spliterator(), false)
                .filter(producto -> producto.getCostoPrducto() == null)
                .toList();
        }

        if ("tipomerchandise-is-null".equals(filter)) {
            log.debug("REST request to get all Productos where tipoMerchandise is null");
            return StreamSupport
                .stream(productoRepository.findAll().spliterator(), false)
                .filter(producto -> producto.getTipoMerchandise() == null)
                .toList();
        }
