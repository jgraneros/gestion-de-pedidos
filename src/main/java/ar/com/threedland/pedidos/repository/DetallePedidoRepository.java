...
public interface DetallePedidoRepository extends JpaRepository<DetallePedido, Long> {
    ...
    @Query(
        value = "select detallePedido from DetallePedido detallePedido left join fetch detallePedido.pedido left join fetch detallePedido.producto",
    ...
    @Query(
        "select detallePedido from DetallePedido detallePedido left join fetch detallePedido.pedido left join fetch detallePedido.producto"
    )
    List<DetallePedido> findAllWithToOneRelationships();

    @Query(
        "select detallePedido from DetallePedido detallePedido left join fetch detallePedido.pedido left join fetch detallePedido.producto where detallePedido.id =:id"
    )
